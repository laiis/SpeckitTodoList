...
// 使用 doMock 並配合 dynamic import 或 require
...
vi.doMock('../../server/services/tokenService', () => ({
  verifyToken: vi.fn(),
  generateToken: vi.fn()
}));
...
describe('Auth & RBAC Middleware Unit Tests', () => {
  ...
  beforeEach(async () => {
    ...
    authorize = rbacModule.default || rbacModule;

    tokenService = await import('../../server/services/tokenService');
  ...
  describe('authMiddleware', () => {
    it('should pass if token is valid (from cookie)', () => {
      ...
      vi.mocked(tokenService.verifyToken).mockReturnValue(decodedUser);
    ...
    it('should fail if token is invalid or expired', () => {
      ...
      vi.mocked(tokenService.verifyToken).mockImplementation(() => { throw new Error('Invalid token'); });
